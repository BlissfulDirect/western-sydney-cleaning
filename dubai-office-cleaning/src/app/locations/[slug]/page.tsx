...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Professional commercial, office, building common areas, medical, school and gym cleaning in ${location.name} ${location.postcode}.`,
  };
}
...
export default async function LocationPage({ params }: Props) {
  ...
  return (
    <>
      <PageHero
        title={`${location.name} Cleaning Services`}
        subtitle={`Commercial, office and specialist cleaning throughout ${location.name} and nearby districts.`}
      ...
      <section className="bg-white py-16 md:py-20">
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <p className="mt-4 leading-relaxed text-neutral-700">
            {site.name} works across {location.name} and the rest of Dubai. We will sit down with you to build a cleaning program that
            suits your site, your hours and your budget — general maintenance or
            a full commercial specification, every two weeks, weekly or daily.
          </p>
          <p className="mt-4 leading-relaxed text-neutral-700">
            Whatever you need — offices, commercial, medical, nursery,
            nursery, building common areas or gym — call us today. Our team can answer
